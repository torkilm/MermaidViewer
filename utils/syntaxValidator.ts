    ...
    return [
      {
        line: 1,
        message:
          'Diagram code is empty. Start with a diagram type like "graph TD" or "sequenceDiagram"',
        severity: 'error',
      },
    ];
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (openBrackets !== closeBrackets) {
      errors.push({
        line: i + 1,
        message:
          'Unmatched brackets [ ]. Make sure each "[" has a matching "]"',
        severity: 'error',
      });
    }
    ...
    if (openParens !== closeParens) {
      errors.push({
        line: i + 1,
        message:
          'Unmatched parentheses ( ). Make sure each "(" has a matching ")"',
        severity: 'error',
      });
    }
    ...
    if (quotes % 2 !== 0) {
      errors.push({
        line: i + 1,
        message:
          'Unclosed quote ". Strings must be enclosed in matching quotes',
        severity: 'error',
      });
    }
  }
