...
export const renderDiagram = async (
  ...
  if (!MERMAID_KEYWORDS.includes(firstWord) && !trimmed.startsWith('%%')) {
    throw new Error(
      `No diagram type detected (like 'graph', 'sequenceDiagram', etc.). Please ensure your code starts with a valid Mermaid keyword.`
    );
  }
  ...
  try {
    ...
    const msg =
      error instanceof Error ? error.message : 'Syntax error in Mermaid code.';
    throw new Error(msg);
  }
};
